...
class Record:#Форматирование записи
    date_format = '%d.%m.%Y'
...
class Calculator:#Первый калькулятор, с созданием списка и добавлением записией
...
class CaloriesCalculator(Calculator): #Калькулятор калорий с проверкой
...
class CashCalculator(Calculator): #Калькулятор кэша с тремя видами валют
    RUB_RATE = 1.0
    USD_RATE = 60.0
    EURO_RATE = 70.0
    ...
    def get_today_cash_remained(self, currency):
        ...
        currency_out = f'{round(abs(left) / currencies[currency][0], 2)} {currencies[currency][1]}'
